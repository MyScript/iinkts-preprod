...
export class IIConnectorManager extends IIAbstractManager
{
  ...
  {
    this.editor.renderer.clearElements({ attrs: { role: ANCHOR_HINT_ROLE } })
  }
  ...
  ): { x: number; y: number } | undefined
  {
    ...
    const FAR = 100000
    const edgeSeg = { p1: anchoredPoint, p2: { x: anchoredPoint.x + (dx / len) * FAR, y: anchoredPoint.y + (dy / len) * FAR } }
    for (let i = 0; i < shapeVertices.length; i++) {
      const shapeSeg = { p1: shapeVertices[i], p2: shapeVertices[(i + 1) % shapeVertices.length] }
  ...
  private recomputeAllEntryPoints(edge: TEdge): void
  {
    if (EdgeOps.isLineEdge(edge)) {
      if (edge.startAnchor) {
        ...
        edge.startAnchor.entryPoint = (target && ShapeOps.isShape(target))
      ...
      if (edge.endAnchor) {
        ...
        edge.endAnchor.entryPoint = (target && ShapeOps.isShape(target))
  ...
  {
    if (!EdgeOps.isLineEdge(edge) && !EdgeOps.isPolyEdge(edge)) return
    ...
    if (target !== undefined) {
      ...
      if (isStart) edge.startAnchor = undefined
      if (isEnd) edge.endAnchor = undefined
    }
  ...
  drawAnchoredEdgesForMatrix(symbolIds: string[], matrix: MatrixTransform): void
  {
    ...
    const recomputeAnchor = (anchor: TAnchor, from: TPoint, to: TPoint): TAnchor =>
    {
      ...
      if (!target || !ShapeOps.isShape(target)) return anchor
      const vertices = idSet.has(anchor.symbolId)
        ? (target as TShape).vertices.map(v => matrix.applyToPoint(v))
        : (target as TShape).vertices
      return { ...anchor, entryPoint: this.computeEntryPoint(from, to, vertices) }
    }

    this.model.symbols.forEach(symbol =>
    {
      if (!EdgeOps.isEdge(symbol)) return
      if (!EdgeOps.isLineEdge(symbol) && !EdgeOps.isPolyEdge(symbol)) return
      ...
      if (EdgeOps.isLineEdge(symbol)) {
        ...
        let end = symbol.end
        const startTargetSymbol = symbol.startAnchor && idSet.has(symbol.startAnchor.symbolId)
          ? this.model.getRootSymbol(symbol.startAnchor.symbolId)
          : undefined
        const endTargetSymbol = symbol.endAnchor && idSet.has(symbol.endAnchor.symbolId)
        ...
        if (changed) {
          const cloneStartAnchor = symbol.startAnchor ? recomputeAnchor(symbol.startAnchor, start, end) : symbol.startAnchor
          const cloneEndAnchor = symbol.endAnchor ? recomputeAnchor(symbol.endAnchor, end, start) : symbol.endAnchor
          const clone = { ...symbol, start, end, startAnchor: cloneStartAnchor, endAnchor: cloneEndAnchor }
          EdgeLineOps.updateDerivedFields(clone)
          this.editor.renderer.drawSymbol(clone)
        }
      }
      else if (EdgeOps.isPolyEdge(symbol)) {
        const points = symbol.points.map(p => ({ ...p }))
        const startTargetSymbol = symbol.startAnchor && idSet.has(symbol.startAnchor.symbolId)
          ? this.model.getRootSymbol(symbol.startAnchor.symbolId)
          : undefined
        const endTargetSymbol = symbol.endAnchor && idSet.has(symbol.endAnchor.symbolId)
        ...
        if (changed) {
          ...
          const cloneStartAnchor = (symbol.startAnchor && n >= 2) ? recomputeAnchor(symbol.startAnchor, points[0], points[1]) : symbol.startAnchor
          const cloneEndAnchor = (symbol.endAnchor && n >= 2) ? recomputeAnchor(symbol.endAnchor, points[n - 1], points[n - 2]) : symbol.endAnchor
          const clone = { ...symbol, points, startAnchor: cloneStartAnchor, endAnchor: cloneEndAnchor }
  ...
  clearAnchoredEdgesFor(symbols: TSymbol[]): void
  {
    symbols.forEach(symbol =>
    {
      if (!EdgeOps.isEdge(symbol)) return
      if (!EdgeOps.isLineEdge(symbol) && !EdgeOps.isPolyEdge(symbol)) return
      if (!symbol.startAnchor && !symbol.endAnchor) return
      symbol.startAnchor = undefined
      symbol.endAnchor = undefined
      if (EdgeOps.isLineEdge(symbol)) EdgeLineOps.updateDerivedFields(symbol)
      else EdgePolyLineOps.updateDerivedFields(symbol)
