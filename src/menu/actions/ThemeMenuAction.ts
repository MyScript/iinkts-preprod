...
export class ThemeMenuAction extends SubMenuItem
{
  ...
  {
    if (!this.subMenuWrapper) return

    this.themes.forEach(theme => {
      const item = this.dom.div({ className: "ms-theme-item" })
      item.setAttribute(STORAGE_KEY_ATTR, theme.id)

      const swatch = this.dom.div({ className: "ms-theme-swatch" })
      swatch.style.backgroundColor = theme.swatch

      const label = this.dom.span({ text: theme.label })
