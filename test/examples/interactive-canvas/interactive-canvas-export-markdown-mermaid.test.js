...
test.describe("Interactive ink canvas Markdown / Diagram / LLM Export", () => {
  ...
  test("should start with all export results empty", async ({ page }) => {
  ...
  test("should clear all export results", async ({ page }) => {
    await Promise.all([
      waitForSynchronizedEvent(page),
      writeStrokes(page, helloOneStroke.strokes),
    ])
