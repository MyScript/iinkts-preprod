import { IIShapeEllipseHelper } from "../../../../src/symbol/helpers/IIShapeEllipseHelper"
import
{
  TPoint,
  DefaultStyle,
...
describe("IIShapeEllipse.ts", () =>
{
  describe("constructor", () =>
  {
    test("should create ", () =>
    {
      ...
      const phi = 0
      const ellipse = IIShapeEllipseHelper.create(center, radiusX, radiusY, phi, style)
    ...
    test("should create with default style", () =>
    {
      ...
      const radiusY = 10
      const phi = 0
      const ellipse = IIShapeEllipseHelper.create(center, radiusX, radiusY, phi)
  ...
  describe("createBetweenPoints", () =>
  {
    test("should create", () =>
    {
      ...
      const ellipse = IIShapeEllipseHelper.createBetweenPoints(origin, target, style)
      expect(ellipse).toBeDefined()
      expect(ellipse.creationTime).toBeLessThanOrEqual(Date.now())
      expect(ellipse.creationTime).toEqual(ellipse.modificationDate)
      expect(ellipse.style).toEqual(expect.objectContaining(style))
  ...
  describe("updateFrupdateBetweenPointsomLine", () =>
  {
    ...
    test("should updateBetweenPoints when target x increas", () =>
    {
      ...
      expect(ellipse.bounds.x).toEqual(1)
      expect(ellipse.bounds.y).toEqual(2)
      IIShapeEllipseHelper.updateBetweenPoints(ellipse, origin, { x: target.x + 6, y: target.y })
    ...
    {
      IIShapeEllipseHelper.updateBetweenPoints(ellipse, origin, { x: target.x, y: target.y + 4 })
  ...
  describe("overlaps", () =>
  {
    ...
    test(`should return true if partially wrap`, () =>
    {
      ...
      expect(IIShapeEllipseHelper.overlaps(ellipse, boundaries)).toEqual(true)
    })
    ...
    test(`should return false if box is outside`, () =>
    {
      ...
      expect(IIShapeEllipseHelper.overlaps(ellipse, boundaries)).toEqual(false)
    })
