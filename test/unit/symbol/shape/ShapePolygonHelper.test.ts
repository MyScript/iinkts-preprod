...
describe("ShapePolygonHelper", () =>
{
  describe("create", () =>
  {
    ...
    test("should create with custom style", () =>
    {
      ...
      const polygon = ShapePolygonHelper.create(points, style)
    ...
    {
      const p1 = ShapePolygonHelper.create(points)
      const p2 = ShapePolygonHelper.create(points)
  ...
  describe("createFromPartial", () =>
  {
    test("should create from valid partial", () =>
    {
      ...
      const polygon = ShapePolygonHelper.createFromPartial({ points: pts })
      expect(polygon.points).toEqual(pts)
    })
    test("should preserve id", () =>
    {
      const pts: TPoint[] = [{ x: 0, y: 0 }, { x: 5, y: 0 }, { x: 2, y: 5 }]
      const polygon = ShapePolygonHelper.createFromPartial({ id: "poly-id", points: pts })
      expect(polygon.id).toEqual("poly-id")
    })
    test("should throw if fewer than 3 points", () =>
    {
      expect(() => ShapePolygonHelper.createFromPartial({ points: [{ x: 0, y: 0 }, { x: 1, y: 1 }] })).toThrow()
    })
  })
  ...
  describe("updateDerivedFields", () =>
  {
    test("should refresh vertices and bounds after points change", () =>
    {
      ...
      const polygon = ShapePolygonHelper.create(pts)
  ...
  describe("overlaps", () =>
  {
    ...
    test("should return true if partially intersects", () =>
    {
      ...
      expect(ShapePolygonHelper.overlaps(polygon, box)).toEqual(true)
    })
    ...
    test("should return false if box is outside", () =>
    {
      ...
      expect(ShapePolygonHelper.overlaps(polygon, box)).toEqual(false)
    })
  ...
  describe("createTriangleBetweenPoints", () =>
  {
    ...
    {
      const polygon = ShapePolygonHelper.createTriangleBetweenPoints({ x: 0, y: 0 }, { x: 10, y: 10 })
  ...
  describe("createRectangleBetweenPoints", () =>
  {
    ...
    {
      const polygon = ShapePolygonHelper.createRectangleBetweenPoints({ x: 0, y: 0 }, { x: 10, y: 10 })
      expect(polygon.points).toHaveLength(4)
    })
    test("should update rectangle between points", () =>
    {
      const polygon = ShapePolygonHelper.createRectangleBetweenPoints({ x: 0, y: 0 }, { x: 5, y: 5 })
      ShapePolygonHelper.updateRectangleBetweenPoints(polygon, { x: 0, y: 0 }, { x: 20, y: 20 })
  ...
  describe("createParallelogramBetweenPoints", () =>
  {
    test("should create 4 points", () =>
    {
      const polygon = ShapePolygonHelper.createParallelogramBetweenPoints({ x: 0, y: 0 }, { x: 10, y: 10 })
      expect(polygon.points).toHaveLength(4)
    })
  })
  ...
  describe("createRhombusBetweenPoints", () =>
  {
    test("should create 4 points", () =>
    {
      const polygon = ShapePolygonHelper.createRhombusBetweenPoints({ x: 0, y: 0 }, { x: 10, y: 10 })
      expect(polygon.points).toHaveLength(4)
    })
  })
})
