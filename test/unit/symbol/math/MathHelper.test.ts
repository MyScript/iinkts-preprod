...
describe("MathHelper", () =>
{
  describe("create", () =>
  {
    ...
    {
      const math = MathHelper.create(elements, point, bounds)
  ...
  describe("createFromPartial", () =>
  {
    ...
    {
      const math = MathHelper.createFromPartial({ elements, point, bounds })
    ...
    {
      expect(() => MathHelper.createFromPartial({ elements: [], point, bounds })).toThrow()
    })

    test("should throw when point is missing", () =>
    {
      expect(() => MathHelper.createFromPartial({ elements })).toThrow()
    })

    test("should throw when bounds is missing", () =>
    {
      expect(() => MathHelper.createFromPartial({ elements, point })).toThrow()
    })

    test("should set custom id from partial", () =>
    {
      const math = MathHelper.createFromPartial({ id: "my-math", elements, point, bounds })
  ...
  describe("overlaps", () =>
  {
    test("should return true when box contains a vertex", () =>
    {
      ...
      expect(MathHelper.overlaps(math, box)).toBe(true)
    })
    ...
    test("should return false when box is completely outside", () =>
    {
      ...
      expect(MathHelper.overlaps(math, box)).toBe(false)
    })
  ...
  describe("getChildrenOverlaps", () =>
  {
    test("should return elements that overlap with given points", () =>
    {
      ...
      const result = MathHelper.getChildrenOverlaps(math, [{ x: 5, y: 8 }])
    ...
    {
      const math = MathHelper.create(elements, point, bounds)
      const result = MathHelper.getChildrenOverlaps(math, [{ x: 1000, y: 1000 }])
    ...
    {
      const math = MathHelper.create(elements, point, bounds)
      const result = MathHelper.getChildrenOverlaps(math, [
        { x: 5, y: 8 },
        { x: 18, y: 8 }
      ])
  ...
  describe("updateChildrenStyle", () =>
  {
    ...
    {
      const math = MathHelper.create(structuredClone(elements), point, bounds)
  ...
  describe("updateChildrenFont", () =>
  {
    ...
    {
      const math = MathHelper.create(structuredClone(elements), point, bounds)
      MathHelper.updateChildrenFont(math, { fontSize: 24 })
    ...
    {
      const math = MathHelper.create(structuredClone(elements), point, bounds)
      MathHelper.updateChildrenFont(math, { fontWeight: "bold" })
    ...
    {
      const math = MathHelper.create(structuredClone(elements), point, bounds)
      MathHelper.updateChildrenFont(math, { fontFamily: "Times" })
    ...
    {
      const math = MathHelper.create(structuredClone(elements), point, bounds)
      MathHelper.updateChildrenFont(math, { fontSize: 20 })
  ...
  describe("getLabel", () =>
  {
    ...
    {
      const math = MathHelper.create(elements, point, bounds)
      expect(MathHelper.getLabel(math)).toBe("x+")
    })

    test("should return empty string for empty elements", () =>
    {
      const math = MathHelper.create([], point, bounds)
      expect(MathHelper.getLabel(math)).toBe("")
    })
  })
})
