...
describe("EdgeArcOps", () => {
  ...
  describe("createFromPartial", () => {
    ...
    test("should throw if center missing", () => {
      expect(() =>
        EdgeArcOps.createFromPartial({ startAngle: 0, sweepAngle: Math.PI, radiusX: 5, radiusY: 5 })
      ).toThrow()
    })
    test("should throw if startAngle missing", () => {
      expect(() =>
        EdgeArcOps.createFromPartial({ center: { x: 0, y: 0 }, sweepAngle: Math.PI, radiusX: 5, radiusY: 5 })
      ).toThrow()
    })
  })
