...
describe("IIEdgeLineHelper", () =>
{
  describe("create", () =>
  {
    ...
    test("should create with custom style", () =>
    {
      ...
      const line = IIEdgeLineHelper.create({ x: 0, y: 0 }, { x: 5, y: 5 }, undefined, undefined, style)
      expect(line.style).toEqual(expect.objectContaining(style))
    })
    test("should create with decorations", () =>
    {
      const line = IIEdgeLineHelper.create({ x: 0, y: 0 }, { x: 5, y: 5 }, EdgeDecoration.Arrow, EdgeDecoration.Arrow)
    ...
    {
      const line = IIEdgeLineHelper.create({ x: 0, y: 0 }, { x: 5, y: 5 })
    ...
    {
      const line = IIEdgeLineHelper.create({ x: 0, y: 0 }, { x: 5, y: 5 }, undefined, undefined, { width: 20 })
    ...
    {
      const l1 = IIEdgeLineHelper.create({ x: 0, y: 0 }, { x: 1, y: 1 })
      const l2 = IIEdgeLineHelper.create({ x: 0, y: 0 }, { x: 1, y: 1 })
  ...
  describe("createFromPartial", () =>
  {
    test("should create from valid partial", () =>
    {
      ...
      const line = IIEdgeLineHelper.createFromPartial(partial)
    ...
    {
      expect(() => IIEdgeLineHelper.createFromPartial({ end: { x: 5, y: 5 } })).toThrow()
    })
    test("should throw if end missing", () =>
    {
      expect(() => IIEdgeLineHelper.createFromPartial({ start: { x: 0, y: 0 } })).toThrow()
    })
  })
  ...
  describe("getResizePoints", () =>
  {
    ...
    {
      const line = IIEdgeLineHelper.create({ x: 0, y: 0 }, { x: 10, y: 10 })
      const pts = IIEdgeLineHelper.getResizePoints(line)
  ...
  describe("overlaps", () =>
  {
    const line = IIEdgeLineHelper.create({ x: 0, y: 0 }, { x: 0, y: 25 })
    test("should return true if partially intersects", () =>
    {
      const box: TBox = { height: 10, width: 10, x: -5, y: -5 }
      expect(IIEdgeLineHelper.overlaps(line, box)).toEqual(true)
    })
    ...
    test("should return false if box is outside", () =>
    {
      ...
      expect(IIEdgeLineHelper.overlaps(line, box)).toEqual(false)
    })
  })
})
