...
describe("IIShapeEllipseHelper", () =>
{
  describe("create", () =>
  {
    ...
    test("should create with custom style", () =>
    {
      ...
      const ellipse = IIShapeEllipseHelper.create({ x: 0, y: 0 }, 5, 10, 0, style)
      expect(ellipse.style).toEqual(expect.objectContaining(style))
    })
    test("should compute bounds from vertices", () =>
    {
      const ellipse = IIShapeEllipseHelper.create({ x: 5, y: 0 }, 5, 10, 0)
    ...
    {
      const ellipse = IIShapeEllipseHelper.create({ x: 0, y: 0 }, 50, 100, 0)
      expect(ellipse.vertices).toHaveLength(50)
    })
    test("should generate unique ids", () =>
    {
      const e1 = IIShapeEllipseHelper.create({ x: 0, y: 0 }, 5, 5, 0)
      const e2 = IIShapeEllipseHelper.create({ x: 0, y: 0 }, 5, 5, 0)
  ...
  describe("createFromPartial", () =>
  {
    test("should create from valid partial", () =>
    {
      ...
      const ellipse = IIShapeEllipseHelper.createFromPartial(partial)
    ...
    {
      expect(() => IIShapeEllipseHelper.createFromPartial({ radiusX: 5, radiusY: 5 })).toThrow()
    })
    test("should throw if radiusX missing", () =>
    {
      expect(() => IIShapeEllipseHelper.createFromPartial({ center: { x: 0, y: 0 }, radiusY: 5 })).toThrow()
    })
    test("should throw if radiusY missing", () =>
    {
      expect(() => IIShapeEllipseHelper.createFromPartial({ center: { x: 0, y: 0 }, radiusX: 5 })).toThrow()
    })
  })
  ...
  describe("overlaps", () =>
  {
    ...
    test("should return true if partially intersects", () =>
    {
      ...
      expect(IIShapeEllipseHelper.overlaps(ellipse, box)).toEqual(true)
    })
    ...
    test("should return false if box is outside", () =>
    {
      ...
      expect(IIShapeEllipseHelper.overlaps(ellipse, box)).toEqual(false)
    })
  ...
  describe("createBetweenPoints", () =>
  {
    test("should set center as midpoint", () =>
    {
      ...
      const ellipse = IIShapeEllipseHelper.createBetweenPoints(origin, target)
    ...
    test("should create with default style", () =>
    {
      const ellipse = IIShapeEllipseHelper.createBetweenPoints({ x: 0, y: 0 }, { x: 4, y: 6 })
      expect(ellipse.style).toEqual(DefaultStyle)
    })
    test("should create with zero radii when origin equals target", () =>
    {
      const pt: TPoint = { x: 1, y: 2 }
      const ellipse = IIShapeEllipseHelper.createBetweenPoints(pt, pt)
  ...
  describe("updateBetweenPoints", () =>
  {
    ...
    {
      const origin: TPoint = { x: 0, y: 0 }
      const ellipse = IIShapeEllipseHelper.createBetweenPoints(origin, { x: 4, y: 6 })
      IIShapeEllipseHelper.updateBetweenPoints(ellipse, origin, { x: 10, y: 20 })
      expect(ellipse.center).toEqual({ x: 5, y: 10 })
      expect(ellipse.radiusX).toEqual(5)
      expect(ellipse.radiusY).toEqual(10)
    })
