...
describe("IIEdgePolyLineHelper", () =>
{
  describe("create", () =>
  {
    ...
    {
      const line = IIEdgePolyLineHelper.create(points, EdgeDecoration.Arrow, EdgeDecoration.Arrow)
    ...
    {
      const line = IIEdgePolyLineHelper.create(points, undefined, undefined, { width: 20 })
    ...
    {
      const l1 = IIEdgePolyLineHelper.create(points)
      const l2 = IIEdgePolyLineHelper.create(points)
  ...
  describe("createFromPartial", () =>
  {
    test("should create from valid partial", () =>
    {
      ...
      const line = IIEdgePolyLineHelper.createFromPartial({ points: pts })
      expect(line.points).toEqual(pts)
    })
    test("should preserve id", () =>
    {
      const pts: TPoint[] = [{ x: 0, y: 0 }, { x: 5, y: 5 }]
      const line = IIEdgePolyLineHelper.createFromPartial({ id: "pl-id", points: pts })
  ...
  describe("updateDerivedFields", () =>
  {
    test("should recompute after points change", () =>
    {
      ...
      const line = IIEdgePolyLineHelper.create(pts)
      line.points = [{ x: 0, y: 0 }, { x: 50, y: 0 }, { x: 50, y: 50 }]
      IIEdgePolyLineHelper.updateDerivedFields(line)
      expect(line.vertices).toBe(line.points)
  ...
  describe("getResizePoints", () =>
  {
    test("should return one resize point per vertex", () =>
    {
      const pts: TPoint[] = [{ x: 0, y: 0 }, { x: 5, y: 0 }, { x: 5, y: 5 }]
      const line = IIEdgePolyLineHelper.create(pts)
      const rp = IIEdgePolyLineHelper.getResizePoints(line)
  ...
  describe("overlaps", () =>
  {
    ...
    test("should return true if partially intersects", () =>
    {
      ...
      expect(IIEdgePolyLineHelper.overlaps(line, box)).toEqual(true)
    })
    ...
    test("should return false if box is outside", () =>
    {
      ...
      expect(IIEdgePolyLineHelper.overlaps(line, box)).toEqual(false)
    })
  })
})
