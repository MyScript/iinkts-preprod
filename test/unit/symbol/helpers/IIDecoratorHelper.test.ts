...
import { SymbolType } from "../../../../src/symbol/base/Symbol"

describe("IIDecoratorHelper", () =>
{
  describe("create", () =>
  {
    test("should create a decorator with required fields", () =>
    {
      const d = IIDecoratorHelper.create(DecoratorKind.Underline, { color: "red", width: 2 })
    ...
    {
      const d = IIDecoratorHelper.create(DecoratorKind.Highlight, { color: "blue" })
    ...
    {
      const ids = ["id1", "id2"]
      const d = IIDecoratorHelper.create(DecoratorKind.Surround, {}, ids)
    ...
    test("should call setBounds when bounds provided", () =>
    {
      ...
      const d = IIDecoratorHelper.create(DecoratorKind.Strikethrough, {}, [], bounds)
    ...
    {
      const d = IIDecoratorHelper.create(DecoratorKind.Underline, {})
    ...
    test("should set numeric creationTime and modificationDate", () =>
    {
      ...
      const d = IIDecoratorHelper.create(DecoratorKind.Highlight, {})
    ...
    {
      const d = IIDecoratorHelper.create(DecoratorKind.Highlight, { opacity: 0.5 })
  ...
  describe("setBounds", () =>
  {
    test("should set bounds and hasBounds", () =>
    {
      ...
      IIDecoratorHelper.setBounds(d, bounds)
      expect(d.bounds).toEqual(bounds)
      expect(d.hasBounds).toBe(true)
    })
    ...
    {
      const d = IIDecoratorHelper.create(DecoratorKind.Underline, {})
      IIDecoratorHelper.setBounds(d, { x: 0, y: 0, width: 50, height: 10 })
    ...
    test("should overwrite previous bounds when called again", () =>
    {
      ...
      IIDecoratorHelper.setBounds(d, newBounds)
  ...
  describe("overlaps", () =>
  {
    test("should return false when hasBounds is false", () =>
    {
      const d = IIDecoratorHelper.create(DecoratorKind.Highlight, {})
      expect(d.hasBounds).toBe(false)
      expect(IIDecoratorHelper.overlaps(d, { x: 0, y: 0, width: 200, height: 200 })).toBe(false)
    })

    test("should return true when bounds overlap the query box", () =>
    {
      const d = IIDecoratorHelper.create(DecoratorKind.Highlight, {})
      IIDecoratorHelper.setBounds(d, { x: 10, y: 10, width: 50, height: 20 })
      expect(IIDecoratorHelper.overlaps(d, { x: 0, y: 0, width: 30, height: 30 })).toBe(true)
    })

    test("should return false when bounds do not overlap the query box", () =>
    {
      const d = IIDecoratorHelper.create(DecoratorKind.Highlight, {})
      IIDecoratorHelper.setBounds(d, { x: 200, y: 200, width: 50, height: 20 })
      expect(IIDecoratorHelper.overlaps(d, { x: 0, y: 0, width: 100, height: 100 })).toBe(false)
    })
  })
})
