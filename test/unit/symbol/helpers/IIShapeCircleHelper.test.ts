...
describe("IIShapeCircleHelper", () =>
{
  describe("create", () =>
  {
    ...
    test("should create with custom style", () =>
    {
      ...
      const circle = IIShapeCircleHelper.create({ x: 0, y: 0 }, 10, style)
      expect(circle.style).toEqual(expect.objectContaining(style))
    })
    test("should compute bounds as center±radius", () =>
    {
      const circle = IIShapeCircleHelper.create({ x: 5, y: 0 }, 5)
    ...
    {
      const circle = IIShapeCircleHelper.create({ x: 0, y: 0 }, 50)
      expect(circle.vertices).toHaveLength(31)
    })
    test("should generate unique ids", () =>
    {
      const c1 = IIShapeCircleHelper.create({ x: 0, y: 0 }, 5)
      const c2 = IIShapeCircleHelper.create({ x: 0, y: 0 }, 5)
  ...
  describe("createFromPartial", () =>
  {
    test("should create from valid partial", () =>
    {
      ...
      const circle = IIShapeCircleHelper.createFromPartial(partial)
    ...
    {
      expect(() => IIShapeCircleHelper.createFromPartial({ radius: 5 })).toThrow()
    })
    test("should throw if radius missing", () =>
    {
      expect(() => IIShapeCircleHelper.createFromPartial({ center: { x: 0, y: 0 } })).toThrow()
    })
  })
  ...
  describe("updateDerivedFields", () =>
  {
    ...
    {
      const circle = IIShapeCircleHelper.create({ x: 0, y: 0 }, 10)
  ...
  describe("overlaps", () =>
  {
    const circle = IIShapeCircleHelper.create({ x: 10, y: 10 }, 10)
    test("should return true if box partially intersects", () =>
    {
      const box: TBox = { height: 10, width: 10, x: -5, y: -5 }
      expect(IIShapeCircleHelper.overlaps(circle, box)).toEqual(true)
    })
    ...
    test("should return false if box is outside", () =>
    {
      ...
      expect(IIShapeCircleHelper.overlaps(circle, box)).toEqual(false)
    })
  ...
  describe("createBetweenPoints", () =>
  {
    test("should create with center at origin", () =>
    {
      ...
      const circle = IIShapeCircleHelper.createBetweenPoints(origin, target)
      expect(circle.center).toEqual(origin)
      expect(circle.radius).toEqual(5)
    })
    test("should create with default style", () =>
    {
      const circle = IIShapeCircleHelper.createBetweenPoints({ x: 0, y: 0 }, { x: 1, y: 0 })
      expect(circle.style).toEqual(DefaultStyle)
    })
    test("should create with zero radius when origin equals target", () =>
    {
      const pt: TPoint = { x: 5, y: 5 }
      const circle = IIShapeCircleHelper.createBetweenPoints(pt, pt)
  ...
  describe("updateBetweenPoints", () =>
  {
    ...
    {
      const origin: TPoint = { x: 0, y: 0 }
      const circle = IIShapeCircleHelper.createBetweenPoints(origin, { x: 3, y: 4 })
      expect(circle.radius).toEqual(5)
      IIShapeCircleHelper.updateBetweenPoints(circle, origin, { x: 6, y: 8 })
      expect(circle.radius).toEqual(10)
    })
    test("should keep center at origin", () =>
    {
      const origin: TPoint = { x: 2, y: 3 }
      const circle = IIShapeCircleHelper.createBetweenPoints(origin, { x: 5, y: 7 })
      IIShapeCircleHelper.updateBetweenPoints(circle, origin, { x: 10, y: 3 })
      expect(circle.center).toEqual(origin)
    })
  })
})
