...
describe("IIEdgeArcHelper", () =>
{
  describe("create", () =>
  {
    test("should create with default style", () =>
    {
      ...
      const arc = IIEdgeArcHelper.create(center, Math.PI / 4, Math.PI / 2, 10, 10, 0)
    ...
    test("should create with custom style", () =>
    {
      ...
      const arc = IIEdgeArcHelper.create({ x: 0, y: 0 }, 0, Math.PI, 10, 10, 0, undefined, undefined, style)
      expect(arc.style).toEqual(expect.objectContaining(style))
    })
    test("should create with decorations", () =>
    {
      const arc = IIEdgeArcHelper.create({ x: 0, y: 0 }, 0, Math.PI, 10, 10, 0, EdgeDecoration.Arrow, EdgeDecoration.Arrow)
    ...
    {
      const arc = IIEdgeArcHelper.create({ x: 0, y: 0 }, Math.PI / 4, 3 * Math.PI / 4, 10, 50, 0, undefined, undefined, { width: 20 })
    ...
    {
      const arc = IIEdgeArcHelper.create({ x: 0, y: 0 }, Math.PI / 4, Math.PI / 4, 5, 5, 0)
      expect(arc.vertices).toHaveLength(9)
    })
    test("should generate unique ids", () =>
    {
      const a1 = IIEdgeArcHelper.create({ x: 0, y: 0 }, 0, Math.PI, 5, 5, 0)
      const a2 = IIEdgeArcHelper.create({ x: 0, y: 0 }, 0, Math.PI, 5, 5, 0)
  ...
  describe("createFromPartial", () =>
  {
    test("should create from valid partial", () =>
    {
      ...
      const arc = IIEdgeArcHelper.createFromPartial(partial)
    ...
    {
      expect(() => IIEdgeArcHelper.createFromPartial({ startAngle: 0, sweepAngle: Math.PI, radiusX: 5, radiusY: 5 })).toThrow()
    })
    test("should throw if startAngle missing", () =>
    {
      expect(() => IIEdgeArcHelper.createFromPartial({ center: { x: 0, y: 0 }, sweepAngle: Math.PI, radiusX: 5, radiusY: 5 })).toThrow()
    })
  })
  ...
  describe("computeVertices", () =>
  {
    ...
    {
      const arc = IIEdgeArcHelper.create({ x: 0, y: 0 }, Math.PI / 4, Math.PI / 4, 5, 5, 0)
      const v = IIEdgeArcHelper.computeVertices(arc)
  ...
  describe("updateDerivedFields", () =>
  {
    ...
    {
      const arc = IIEdgeArcHelper.create({ x: 0, y: 0 }, 0, Math.PI, 10, 10, 0)
  ...
  describe("getResizePoints", () =>
  {
    ...
    {
      const arc = IIEdgeArcHelper.create({ x: 0, y: 0 }, Math.PI / 4, Math.PI / 2, 10, 10, 0)
      const pts = IIEdgeArcHelper.getResizePoints(arc)
  ...
  describe("overlaps", () =>
  {
    const arc = IIEdgeArcHelper.create({ x: 0, y: 0 }, Math.PI / 4, Math.PI / 2, 10, 50, 0)
    test("should return true if partially intersects", () =>
    {
      const box: TBox = { height: 20, width: 20, x: 0, y: 45 }
      expect(IIEdgeArcHelper.overlaps(arc, box)).toEqual(true)
    })
    ...
    test("should return false if box is outside", () =>
    {
      ...
      expect(IIEdgeArcHelper.overlaps(arc, box)).toEqual(false)
    })
  })
})
