...
describe("IITextHelper", () =>
{
  describe("create", () =>
  {
    test("should return a plain object with correct type", () =>
    {
      const text = IITextHelper.create(chars, point, bounds)
  ...
  describe("createFromPartial", () =>
  {
    ...
    {
      const text = IITextHelper.createFromPartial({ chars, point, bounds })
    ...
    {
      expect(() => IITextHelper.createFromPartial({ chars: [], point, bounds })).toThrow()
    })

    test("should throw when point is missing", () =>
    {
      expect(() => IITextHelper.createFromPartial({ chars })).toThrow()
    })

    test("should throw when bounds is missing", () =>
    {
      expect(() => IITextHelper.createFromPartial({ chars, point })).toThrow()
    })

    test("should set custom id from partial", () =>
    {
      const text = IITextHelper.createFromPartial({ id: "my-id", chars, point, bounds })
  ...
  describe("overlaps", () =>
  {
    test("should return true when box contains a vertex", () =>
    {
      ...
      expect(IITextHelper.overlaps(text, box)).toBe(true)
    })
    ...
    test("should return false when box is completely outside", () =>
    {
      ...
      expect(IITextHelper.overlaps(text, box)).toBe(false)
    })
  ...
  describe("getChildrenOverlaps", () =>
  {
    test("should return chars that overlap with given points", () =>
    {
      ...
      const result = IITextHelper.getChildrenOverlaps(text, [{ x: 5, y: 10 }])
    ...
    {
      const text = IITextHelper.create(chars, point, bounds)
      const result = IITextHelper.getChildrenOverlaps(text, [{ x: 1000, y: 1000 }])
    ...
    test("should return all chars when all overlap", () =>
    {
      ...
      const result = IITextHelper.getChildrenOverlaps(text, [
        { x: 5, y: 10 },
        { x: 13, y: 10 }
      ])
  ...
  describe("updateChildrenStyle", () =>
  {
    ...
    {
      const text = IITextHelper.create(structuredClone(chars), point, bounds)
      text.style.color = "#FF0000"
      IITextHelper.updateChildrenStyle(text)
  ...
  describe("updateChildrenFont", () =>
  {
    ...
    {
      const text = IITextHelper.create(structuredClone(chars), point, bounds)
      IITextHelper.updateChildrenFont(text, { fontSize: 24 })
    ...
    {
      const text = IITextHelper.create(structuredClone(chars), point, bounds)
      IITextHelper.updateChildrenFont(text, { fontWeight: "bold" })
    ...
    {
      const text = IITextHelper.create(structuredClone(chars), point, bounds)
      IITextHelper.updateChildrenFont(text, { fontSize: 20 })
  ...
  describe("getLabel", () =>
  {
    ...
    {
      const text = IITextHelper.create(chars, point, bounds)
      expect(IITextHelper.getLabel(text)).toBe("Hi")
    })

    test("should return empty string for empty chars", () =>
    {
      const text = IITextHelper.create([], point, bounds)
      expect(IITextHelper.getLabel(text)).toBe("")
    })
  })
})
