...
describe("HmacAuth.ts", () => {
  ...
  test("should return undefined when no hmacKey is configured", async () => {
    const result = await resolveHmac({ ...baseServer, hmacKey: "" }, { some: "data" })
  ...
  test("should compute the hmac from a string hmacKey", async () => {
    ...
    const expected = await computeHmac(JSON.stringify(data), server.applicationKey, server.hmacKey as string)
  ...
  test("should compute the hmac from an async hmacKey resolver function", async () => {
    ...
    const data = { some: "data" }
    const expected = await computeHmac(JSON.stringify(data), server.applicationKey, "resolved-app-key")
    const result = await resolveHmac(server, data)
    expect(result).toEqual(expected)
  })
  ...
  test("should throw when hmacKey is neither a string nor a function", async () => {
    ...
    await expect(resolveHmac(server, { some: "data" })).rejects.toThrow("HMAC key is not a string nor a function")
  })
})
