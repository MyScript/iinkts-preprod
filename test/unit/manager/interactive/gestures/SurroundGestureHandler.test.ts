...
describe("SurroundGestureHandler.ts", () => {
  ...
  describe("apply", () => {
    test("should handle Select action", async () => {
      ...
      StrokeOps.addPointer(gestureStroke, { x: 5, y: 5, p: 1, dt: 300 })
      StrokeOps.addPointer(gestureStroke, { x: 25, y: 25, p: 1, dt: 400 })
    ...
    test("should handle Highlight action", async () => {
      ...
      StrokeOps.addPointer(gestureStroke, { x: 35, y: 30, p: 1, dt: 200 })
    ...
    test("should handle empty symbols", async () => {
      const gestureStroke = buildIIStroke()
      StrokeOps.addPointer(gestureStroke, { x: 100, y: 100, p: 1, dt: 100 })
      StrokeOps.addPointer(gestureStroke, { x: 200, y: 200, p: 1, dt: 200 })
    ...
    test("should handle unknown surround action", async () => {
      const gestureStroke = buildIIStroke()
      StrokeOps.addPointer(gestureStroke, { x: 10, y: 10, p: 1, dt: 100 })
  ...
  describe("integration", () => {
    test("should select symbols within gesture bounds", async () => {
      ...
      StrokeOps.addPointer(stroke1, { x: 15, y: 15, p: 1, dt: 200 })

      const stroke2 = buildIIStroke()
      StrokeOps.addPointer(stroke2, { x: 100, y: 100, p: 1, dt: 300 })
      StrokeOps.addPointer(stroke2, { x: 105, y: 105, p: 1, dt: 400 })

      canvas.model.addSymbol(stroke1)
      canvas.model.addSymbol(stroke2)

      const gestureStroke = buildIIStroke()
      StrokeOps.addPointer(gestureStroke, { x: 5, y: 5, p: 1, dt: 500 })
      StrokeOps.addPointer(gestureStroke, { x: 20, y: 20, p: 1, dt: 600 })
