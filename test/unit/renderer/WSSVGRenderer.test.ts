...
describe("InteractiveInkSSRSVGRenderer.ts", () => {
  ...
  test("should drawPendingStroke", () => {
    ...
    expect(renderer.stroker.drawStroke).toHaveBeenCalledWith(pendingStrokesGroup, stroke, [
      { name: "style", value: "fill:undefined;stroke:transparent;" },
    ])
  })
  ...
  test("should drawPendingStroke when pointerType = eraser", () => {
    ...
    expect(renderer.stroker.drawStroke).toHaveBeenCalledTimes(1)
    expect(renderer.stroker.drawStroke).toHaveBeenCalledWith(pendingStrokesGroup, eraseStroke, [
      { name: "style", value: "fill:grey;stroke:transparent;shadowBlur:5;opacity:0.2;" },
    ])
  })
  ...
  test("should resize", () => {
    const width = DefaultRendererConfiguration.minWidth,
      height = DefaultRendererConfiguration.minHeight
    const domElement = document.createElement("div") as HTMLElement
    const renderer = new InteractiveInkSSRSVGRenderer(DefaultRendererConfiguration)
